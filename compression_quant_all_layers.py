...
# -----------------------------
# Quantization (all layers, symmetric per channel)

# The layers we want to quantize:
op_name_dict = {}
layers = [n for n, p in model.named_parameters()]  # we take all model layers
for key in layers:
    # Both weights and activations are quantized to int8:
    op_name_dict[key] = {
        "weight": {
            "dtype": ["int8"],
            "algorithm": ["minmax"],  # use an arbitrary quantization algorithm
...
quantize(op_name_dict, 'CodeGPT-Py150-0.6-sparse-q-all-layers-sym-per-channel')

# -----------------------------
# Quantization (all layers, asymmetric per channel)
...
for key in layers:
    op_name_dict[key] = {
        "weight": {
            ...
            "algorithm": ["minmax"],
            "granularity": ["per_channel"],
            "scheme": ["asym"]  # use asymmetric quantization
        },
        "activation": {
            "dtype": ["int8"],
            "algorithm": ["minmax"],
...
quantize(op_name_dict, 'CodeGPT-Py150-0.6-sparse-q-all-layers-asym-per-channel')

# -----------------------------
# Quantization (all layers, asymmetric, per-tensor)
...
for key in layers:
    op_name_dict[key] = {
        "weight": {
            "dtype": ["int8"],
            "algorithm": ["minmax"],
            "granularity": ["per_tensor"],  # per-tensor quantization
            "scheme": ["asym"]
        },
        "activation": {
            "dtype": ["int8"],
...
quantize(op_name_dict, 'CodeGPT-Py150-0.6-sparse-q-all-layers-asym-per-tensor')

# -----------------------------
# Quantization (all layers, symmetric, per-tensor)
...
quantize(op_name_dict, 'CodeGPT-Py150-0.6-sparse-q-all-layers-sym-per-tensor')
